...
#define CPP_THROW_WITH_SOURCE_INFO(exceptionType, message) \
    ...
    { \
        ...
        + "\nexception was thrown at:" \
          ...
          "\nfunction: " \
        + std::string{ CPP_CURRENT_FUNCTION } \
    }
...
#define CPP_THROW_IF_NULL(pointer) \
    ...
    if (pointer == nullptr) \
    { \
        CPP_THROW_WITH_SOURCE_INFO(::cpp::NullPointerException, \
            CPP_STRINGIFY(pointer) " was null!"); \
    } \
    CPP_END_MACRO

#define CPP_NOT_YET_IMPLEMENTED \
    CPP_THROW_WITH_SOURCE_INFO(::cpp::NotYetImplementedException, \
...
#define CPP_DETAIL_ASSERTION_IMPLEMENTATION(condition, exceptionType, violationTypeString) \
    CPP_BEGIN_MACRO \
        if (!(condition)) \
        { \
            CPP_THROW_WITH_SOURCE_INFO(exceptionType, \
...
#define CPP_CHECK_PRE(precondition) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION(precondition, \
        ::cpp::PreconditionViolationException, \
        "PRECONDITION" \
    )

#define CPP_CHECK_PRE_MSG(precondition, message) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION_MSG(precondition, \
        ::cpp::PreconditionViolationException, \
        "PRECONDITION", \
        message \
    )
...
#define CPP_CHECK_POST(postcondition) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION(postcondition, \
        ::cpp::PostconditionViolationException, \
        "POSTCONDITION" \
    )

#define CPP_CHECK_POST_MSG(postcondition, message) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION_MSG(postcondition, \
        ::cpp::PostconditionViolationException, \
        "POSTCONDITION", \
        message \
    )
...
#define CPP_ASSERT(condition) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION(condition, \
        ::cpp::AssertionViolationException, \
        "ASSERTION" \
    )

#define CPP_ASSERT_MSG(condition, message) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION_MSG(condition, \
        ::cpp::AssertionViolationException, \
        "ASSERTION", \
        message \
    )
...
#define CPP_CHECK_BOUNDS(item, lower_bound, upper_bound) \
    CPP_DETAIL_ASSERTION_IMPLEMENTATION((lower_bound <= item) \
                                     && (item <= upper_bound), \
        ::cpp::IndexOutOfBoundsException, \
        "BOUNDSCHECK" \
    )
