...
#    define DOCTEST_TEST_CASE_TEMPLATE_INVOKE_IMPL(id, anon, ...)                                                      \
        DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_CAT(anon, DUMMY)) = [] {                                                    \
            DOCTEST_CAT(id, ITERATOR)<std::tuple<__VA_ARGS__>> DOCTEST_UNUSED DOCTEST_CAT(anon, inner_dummy)(__LINE__, \
                                                                                                             0);       \
            return 0;                                                                                                  \
        }();                                                                                                           \
        DOCTEST_GLOBAL_NO_WARNINGS_END()
...
#    define DOCTEST_TEST_CASE_FIXTURE(x, name)                                                                         \
        DOCTEST_IMPLEMENT_FIXTURE(DOCTEST_ANONYMOUS(_DOCTEST_ANON_CLASS_), x, DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_),  \
                                  name)
...
#        endif // DOCTEST_PLATFORM_WINDOWS
        ...
        struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
        {
            ...
            void finalizeTestCaseData()
            {
                ...
                {
                    if (failure_flags) { failure_flags |= TestCaseFailureReason::ShouldHaveFailedAndDid; }
                    else
                    {
...
#    endif // DOCTEST_CONFIG_DISABLE
    ...
    {
        if (other.isOnStack()) { memcpy(buf, other.buf, len); }
        else
        {
            setOnHeap();
...
#        if defined(DOCTEST_CONFIG_POSIX_SIGNALS) || defined(DOCTEST_CONFIG_WINDOWS_SEH)
        void reportFatal(const std::string& message)
        {
            ...
            while (g_cs->subcasesCurrentLevel--) DOCTEST_ITERATE_THROUGH_REPORTERS(subcase_end, DOCTEST_EMPTY);
        ...
        struct ConsoleReporter : public IReporter
        {
            ...
            void report_query(const QueryData& in) override
            {
                ...
                else if (opt.count || opt.list_test_cases)
                {
                    ...
                    for (unsigned i = 0; i < in.num_data; ++i) s << Color::None << in.data[i] << "\n";
...
#        endif // DOCTEST_CONFIG_NO_EXCEPTIONS
        ...
        if (!query_mode) { DOCTEST_ITERATE_THROUGH_REPORTERS(test_run_end, *g_cs); }
        else
        {
            QueryData qdata;
